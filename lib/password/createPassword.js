...
const createPassword = ash(async (req,res,next)=>{
  ...
  let {url,username,password,name,notes,folder_id}  = req.body;
  ...
  if(passIsValid){
    ...
    createNewPassword.then((newpassword)=>{
      ...
      const findPasswordWithFolder = Password.findOne({
        where:{user_id:newPassword.user_id,id:newpassword.id},
        include:Folder
      })
      findPasswordWithFolder.then((pWithFolder)=>{
        return res.send({
          success:true,
          message:"Password created successfully",
          password:pWithFolder
        })
      }).catch((pwithFolderErr)=>next(pwithFolderErr))
